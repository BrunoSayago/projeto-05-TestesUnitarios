...
let objetoRetornadoCode;

const orderFromMenu = (request) => {
  objetoRetornadoCode.consumption.push(request);
};

const mesclaFoodDrink = (objeto) => {
...
const createMenu = (objeto) => {
  objetoRetornadoCode = { fetchMenu: () => objeto,
  consumption: [],
  order: orderFromMenu,
  pay: () => {
    let soma = 0;
    for (let item of objetoRetornadoCode.consumption) {
        soma += mesclaFoodDrink(objeto)[item];
    }
    let total = soma * 1.1;
    return total;
  },
  };
  return objetoRetornadoCode;
};
