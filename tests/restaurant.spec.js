...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  ...
  it('TESTE 3: Verifique se o menu passado pra função createMenu() é idêntico ao menu recuperado pela função "objetoRetornado.fetchMenu()"', () => {
    const objetoRetornado = createMenu({ food: { brigadeiro: 2.5, bolo: 10.9 }, drink: { refrigerante: 5.0, gin: 11.9 } });

    expect({ food: { brigadeiro: 2.5, bolo: 10.9 }, drink: { refrigerante: 5.0, gin: 11.9 } }).toEqual(objetoRetornado.fetchMenu());
  });
  ...
  it('TESTE 5: Verifique se, ao chamar uma função associada à chave `order` no objeto retornado,passando uma string como parâmetro (como `objetoRetornado.order("coxinha")`), tal string é adicionada ao array retornado em `objetoRetornado.consumption`.', () => {
    ...
    expect(objetoRetornado.consumption).toEqual(['bolo']);
  });
    ...
    it('TESTE 6: Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.', () => {
      const objetoRetornado = createMenu({ food: { brigadeiro: 2.5, bolo: 10.9 }, drink: { refrigerante: 5.0, gin: 11.9 } });
      objetoRetornado.order("bolo");
      objetoRetornado.order("refrigerante");
      objetoRetornado.order("brigadeiro");
      objetoRetornado.order("sushi");
      expect(objetoRetornado.consumption).toEqual(['bolo', 'refrigerante', 'brigadeiro', 'sushi']);
    });
    ...
    it('TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.', () => {
      const objetoRetornado = createMenu({ food: { brigadeiro: 2.5, bolo: 10.9 }, drink: { refrigerante: 5.0, gin: 11.9 } });
      objetoRetornado.order('bolo');
      objetoRetornado.order('gin');
      objetoRetornado.order('bolo');
      expect(objetoRetornado.consumption).toEqual(['bolo', 'gin', 'bolo']);
    });
    ...
    it('TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
      const objetoTeste = { food: { brigadeiro: 2.5, bolo: 10.9 }, drink: { refrigerante: 5.0, gin: 11.9 } };
      const objetoRetornado = createMenu(objetoTeste);
      objetoRetornado.order('bolo');
      objetoRetornado.order('gin');
      objetoRetornado.order('bolo');
      const mesclaFoodDrinkTeste = (objeto) => {
        let foods = objeto.food;
        let drinks = objeto.drink;
        const todas = {};
        Object.assign(todas, foods, drinks);
        return todas;
      };
      const somaPreco = (objeto, arrey) => {
        let soma = 0;
        for (let item of arrey) {
          soma += mesclaFoodDrinkTeste(objeto)[item];
        }
        let total = soma * 1.1;
        return total;
      };
      expect(objetoRetornado.pay()).toEqual(somaPreco(objetoTeste, objetoRetornado.consumption));
    });
});
