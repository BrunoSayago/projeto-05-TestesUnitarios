...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle(2).circumference).toBeCloseTo((Math.PI * 2 * 2), 1);
    expect(circle(3).area).toBeCloseTo((Math.PI * (3 ** 2)), 1);
    expect(circle(3)).toMatchObject({radius: 3, area: (3.14 * 3 * 3), circumference: (3.14 * 2 * 3)});
